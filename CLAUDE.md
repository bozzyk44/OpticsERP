...
### Управление портами и процессами (ОБЯЗАТЕЛЬНО)

**КРИТИЧНО:** ВСЕГДА использовать стандартные порты для сервисов. НИКОГДА не менять порты.

#### Стандартные порты проекта:

| Сервис | Порт | Назначение |
|--------|------|------------|
| **KKT Adapter (FastAPI)** | **8000** | Адаптер ККТ REST API |
| Odoo | 8069 | Odoo web server |
| PostgreSQL | 5432 | База данных Odoo |
...
1. **ВСЕГДА использовать стандартный порт** - не менять порт даже если занят
2. **Если порт занят** - убить процесс на этом порту:
   ```bash
   # Использовать утилиту kill_port.py
   python scripts/kill_port.py 8000

   # Для принудительного убийства (SIGKILL)
   python scripts/kill_port.py 8000 --force

   # Убить все стандартные порты сервисов (8000, 8069, 5432, 6379)
   python scripts/kill_port.py 8000 --all
   ```

3. **Перед запуском сервиса** - проверить, что порт свободен:
   ```bash
   # Проверка порта 8000
   curl -s http://localhost:8000/ > /dev/null 2>&1 && echo "Port 8000 is in use" || echo "Port 8000 is free"
   ```

4. **Не запускать несколько экземпляров** одного сервиса на разных портах
5. **После завершения тестирования** - убить все тестовые процессы

#### Примеры:

**Правильно:**
```bash
# 1. Убить старый процесс FastAPI на порту 8000
python scripts/kill_port.py 8000

# 2. Запустить FastAPI на стандартном порту 8000
cd kkt_adapter/app && python main.py
```

**Неправильно:**
```bash
# ❌ НЕ делать так - не менять порт!
cd kkt_adapter/app && python main.py --port 8001  # WRONG!
```
...
**КРИТИЧНО:** После КАЖДОЙ завершенной задачи (OPTERP-X) ОБЯЗАТЕЛЬНО выполнить commit и push!
...
5. **Push в remote (ОБЯЗАТЕЛЬНО):**
   ```bash
   git push -u origin feature/task-name
   # Или, если ветка уже отслеживается:
   git push
   ```

6. **Создать Pull Request (опционально - только по запросу пользователя):**
   ```bash
...
### Правила Commit и Push (ОБЯЗАТЕЛЬНО)

**Когда делать commit и push:**

| Событие | Действие | Обязательность |
|---------|----------|----------------|
| **Завершена задача (OPTERP-X)** | Commit + Push | ✅ ОБЯЗАТЕЛЬНО |
| **Все тесты пройдены** | Commit + Push | ✅ ОБЯЗАТЕЛЬНО |
| **Добавлен новый функционал** | Commit + Push | ✅ ОБЯЗАТЕЛЬНО |
| **Исправлен баг** | Commit + Push | ✅ ОБЯЗАТЕЛЬНО |
| **Добавлена документация** | Commit + Push | ✅ ОБЯЗАТЕЛЬНО |
| **Завершена сессия работы** | Commit + Push | ✅ ОБЯЗАТЕЛЬНО |
| **Перед переходом к новой задаче** | Commit + Push | ✅ ОБЯЗАТЕЛЬНО |

**ПРАВИЛО:** Никогда не переходить к следующей задаче без commit и push текущей!

### Commit Message Format

**Типы коммитов:**
- `feat(scope):` - новая функциональность
- `test(scope):` - добавление/изменение тестов
- `fix(scope):` - исправление бага
- `docs(scope):` - документация
- `refactor(scope):` - рефакторинг без изменения функциональности
- `chore(scope):` - технические задачи (build, CI/CD)

**Примеры:**
```bash
feat(buffer): implement SQLite buffer CRUD operations [OPTERP-4]
test(hlc): add comprehensive unit tests for HLC [OPTERP-3]
fix(buffer): change Lock to RLock to fix deadlock
docs(testing): add test logging guidelines to CLAUDE.md
```

**Структура commit message:**
```
<type>(<scope>): <краткое описание> [OPTERP-X]
...
- [ ] Нет linter errors
- [ ] Логи тестов сохранены в `tests/logs/`
- [ ] Task plan создан в `docs/task_plans/`
- [ ] Код скомпилирован без ошибок
- [ ] Документация обновлена (если нужно)
- [ ] JIRA task ID указан в commit message

**После commit ОБЯЗАТЕЛЬНО:**
- [ ] `git push` выполнен успешно
- [ ] Изменения видны в remote repository (GitHub)
- [ ] Нет конфликтов с remote веткой

### Автоматизация Push

**Добавить алиас для автоматического commit + push:**
```bash
# В .bashrc или .zshrc
...
**Использование:**
```bash
# После завершения OPTERP-5
git add docs/task_plans/20251008_OPTERP-5_buffer_unit_tests.md tests/unit/test_buffer.py
git commit -m "test(buffer): add comprehensive unit tests [OPTERP-5]"
git push  # ОБЯЗАТЕЛЬНО!
```

### Частота Commit и Push

**Рекомендуемая частота:**

1. **Минимум:** После каждой завершенной задачи (OPTERP-X)
2. **Оптимально:** После каждого логического блока работы
3. **Максимум:** Каждые 2-3 часа работы

**Примеры:**
- ✅ Хорошо: Завершил OPTERP-4 → commit + push → перехожу к OPTERP-5
- ✅ Хорошо: Написал код → написал тесты → commit + push
- ❌ Плохо: Завершил 3 задачи → делаю один commit
- ❌ Плохо: Работаю весь день → commit в конце дня

### Обработка Ошибок Push

**Если `git push` не удался:**
...
3. **Если ветка не отслеживается:**
   ```bash
   git push -u origin feature/task-name
   ```

4. **Если push отклонен (force push НЕ допускается для main):**
   ```bash
   # НЕ ДЕЛАТЬ для main/master:
   # git push --force
...
**❌ НЕ ДЕЛАТЬ:**
1. Завершить задачу и забыть push
2. Делать commit без push "на потом"
3. Накапливать несколько задач перед push
4. Force push в main/master без согласования
5. Commit с message "WIP" или "temp" и забыть исправить
6. Push без проверки тестов

**✅ ДЕЛАТЬ:**
1. Commit + push после КАЖДОЙ задачи
2. Проверять `git status` перед commit
3. Запускать тесты перед commit
4. Писать понятные commit messages
5. Указывать JIRA task ID в message
6. Проверять, что push прошел успешно

### Восстановление После Забытого Push

**Если забыли сделать push:**

```bash
# 1. Проверить статус
git status

# 2. Проверить unpushed commits
git log origin/feature/task-name..HEAD

# 3. Push все unpushed commits
git push

# 4. Проверить, что все синхронизировано
git status
# Должно быть: "Your branch is up to date with 'origin/feature/task-name'"
```

### Важность Push

**Почему push ОБЯЗАТЕЛЕН:**

1. **Backup:** Код сохранен в remote, не потеряется при сбое локальной машины
2. **Видимость:** Другие разработчики (или вы в другой сессии) видят прогресс
3. **CI/CD:** Автоматические тесты запускаются на remote
4. **История:** Полная история изменений доступна в GitHub
5. **Code Review:** Код можно ревьюить через GitHub
6. **Rollback:** Можно откатиться к любому pushed коммиту

**ВАЖНО:** Commit без push = потенциальная потеря работы!
