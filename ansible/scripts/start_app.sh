...
# Function to start KKT Adapter
start_kkt_adapter() {
    echo ""
    echo -e "${BLUE}→${NC} Starting KKT Adapter on kkt_adapters..."
    ansible kkt_adapters -i "$INVENTORY_FILE" -m shell \
        -a "cd /opt/opticserp/kkt_adapter && docker-compose up -d" \
        --become-user deploy || true
...
check_status() {
    ...
    case "$COMPONENT" in
        ...
        kkt-adapter)
            ...
            ansible kkt_adapters -i "$INVENTORY_FILE" -m shell \
                -a "cd /opt/opticserp/kkt_adapter && docker-compose ps" \
                --become-user deploy || true
            ;;
...
show_logs() {
    ...
    case "$service" in
        ...
        kkt-adapter)
            ansible kkt_adapters -i "$INVENTORY_FILE" -m shell \
                -a "cd /opt/opticserp/kkt_adapter && docker-compose logs --tail=$lines" \
                --become-user deploy
            ;;
...
case "$COMPONENT" in
    ...
    *)
        ...
        echo "  kkt-adapter - Start KKT Adapter only"
        echo "  monitoring  - Start Prometheus + Grafana only"
        echo "  status      - Show service status"
        echo "  logs        - Show logs (add service name: odoo, kkt-adapter, prometheus, grafana)"
